...
echo === Отчёты ===
for %%T in (net8.0 net9.0 net10.0) do (
    for %%R in (startup expression) do (
        echo   %%T %%R
...
dotnet run -c Release -f net10.0 --no-build -- --filter *LocalsInitBench* noasm > "%OUT%\probe_noasm.txt" 2>&1
set PROBE1=%ERRORLEVEL%
if exist "Bdn\AttributeProof.log" copy /Y "Bdn\AttributeProof.log" "%OUT%\probe_noasm.log" >nul
if exist "Bdn\*.log" del /Q "Bdn\*.log"
if not "%PROBE1%"=="0" echo   завершилось с ошибкой, смотри "%OUT%\probe_noasm.txt"

echo.
echo === Проба: один класс с дизассемблером ===
dotnet run -c Release -f net10.0 --no-build -- --filter *LocalsInitBench* > "%OUT%\probe_asm.txt" 2>&1
set PROBE2=%ERRORLEVEL%
if exist "Bdn\AttributeProof.log" copy /Y "Bdn\AttributeProof.log" "%OUT%\probe_asm.log" >nul
if exist "Bdn\*.log" del /Q "Bdn\*.log"
...
if exist "Bdn\AttributeProof.log" copy /Y "Bdn\AttributeProof.log" "%OUT%\bench.log" >nul
